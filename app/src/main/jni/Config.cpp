...
Java_com_catherine_securitysample_JNIHelper_getAuthChain(JNIEnv *env, jobject instance,
                                                         jstring key) {

    jobjectArray valueArray = (jobjectArray) env->NewObjectArray(5,
...
Java_com_catherine_securitysample_JNIHelper_getKeyParams(JNIEnv *env, jobject instance) {
    ...
    const char *hash[2];
    //MODULUS
    hash[0] = "AKfszhN0I/O12wcJ+r4wX0Im//5+pGeSFCXo4jOH18khVsspwgDaZgUJRxYIeK87kDOmk8U1j01Rsx2UFlThMjfwT9oliR1K/QihIujN7dgLSnBHh8wWXBI+P+hZq01uF2qrvWZQ+t2JySVBh7DO9uXxdjHrOLou97w3pjZzU4zn";
    //EXPONENT
    hash[1] = "AQAB";
    for (int i = 0; i < 2; i++) {
        jstring value = env->NewStringUTF(hash[i]);
